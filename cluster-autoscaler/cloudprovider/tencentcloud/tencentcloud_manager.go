...
import (
	...
	"k8s.io/autoscaler/cluster-autoscaler/cloudprovider/tencentcloud/credential"
)
...
type TencentcloudManager interface {
	...
	GetAsgForInstance(instance TencentcloudRef) (Asg, error)
	...
	DeleteInstances(instances []TencentcloudRef) error
}
...
type tencentcloudManagerImpl struct {
	...
	regional             bool
	explicitlyConfigured map[TencentcloudRef]bool
...
func readCloudConfig(configReader io.Reader) error {
	...
	if (clusterID != "") && (clusterID != "kube-system") {
		cloudConfig.ClusterId = clusterID
		klog.V(4).Infof("read clusterId from env, clusterId: %s", cloudConfig.ClusterId)
	} else {
		...
		if !section.HasKey("KUBE_CLUSTER") {
			return fmt.Errorf("key(KUBE_CLUSTER) not found")
		}
		cloudConfig.ClusterId = section.Key("KUBE_CLUSTER").String()
		klog.V(6).Info("Read cluster ID from /etc/kubernetes/config")
	}
...
func CreateTencentcloudManager(configReader io.Reader, discoveryOpts cloudprovider.NodeGroupDiscoveryOptions, regional bool) (TencentcloudManager, error) {
	...
	credential.SetService(serviceName)
	innerCredential := credential.NewCredential(credential.InnerUserCredential, tokenExpiredTime)
	serviceCredential := credential.NewCredential(credential.ServiceTokenCredential, tokenExpiredTime)

	cvmClient, err := cvm.NewClient(innerCredential, cloudConfig.RegionName, newCVMClientProfile())
	if err != nil {
		return nil, err
	}
	vpcClient, err := vpc.NewClient(innerCredential, cloudConfig.RegionName, newVPCClientProfile())
	if err != nil {
		return nil, err
	}
	asClient, err := as.NewClient(innerCredential, cloudConfig.RegionName, newASClientProfile())
	if err != nil {
		return nil, err
	}
	tkeClient, err := tke.NewClient(serviceCredential, cloudConfig.RegionName, newTKEClientProfile())
	if err != nil {
		return nil, err
	}
	...
	manager := &tencentcloudManagerImpl{
		...
		explicitlyConfigured: make(map[TencentcloudRef]bool),
	}
...
func (m *tencentcloudManagerImpl) GetAsgForInstance(instance TencentcloudRef) (Asg, error) {
...
// DeleteInstances deletes the given instances. All instances must be controlled by the same ASG.
func (m *tencentcloudManagerImpl) DeleteInstances(instances []TencentcloudRef) error {
	if len(instances) == 0 {
		return nil
	}
...
func (m *tencentcloudManagerImpl) GetAsgNodes(asg Asg) ([]cloudprovider.Instance, error) {
	...
	group, err := m.cloudService.GetAutoScalingInstances(asg.TencentcloudRef())
	if err != nil {
		return result, err
	}
...
func (m *tencentcloudManagerImpl) getAsgTemplate(asgRef TencentcloudRef) (*asgTemplate, error) {
	...
	zoneInfo, err := m.cloudService.GetZoneInfo(instanceInfo.Zone)
	if err != nil {
		return nil, err
	}
...
func (m *tencentcloudManagerImpl) GetAsgTemplateNode(asg Asg) (*apiv1.Node, error) {
	...
	template, err := m.getAsgTemplate(asg.TencentcloudRef())
	if err != nil {
		return nil, err
	}
